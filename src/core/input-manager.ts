...
export class InputManager {
  ...
  // ==========================================
  // ✅ ИСПРАВЛЕНО: expandInputArea с правильным автофокусом
  // ==========================================
  ...
  expandInputArea(): void {
    ...
    // 4. Обновляем состояние кнопки очистки
    if (clearBtn) {
    ...
    // 5. Авто-подстройка высоты textarea
    this._resizeTextArea(userInput, clearBtn);

    // 6. Скрываем нижнюю навигацию
    const nav = document.getElementById('bottom-nav');
    if (nav) {
      nav.style.display = 'none';
    }

    // 7. ✅ ИСПРАВЛЕНО: АВТОФОКУС НА МОБИЛЬНЫХ
    //    Даём браузеру время на рендеринг, затем фокусируемся
    ...
    // Первый animation frame — браузер готовится к перерисовке
    requestAnimationFrame(() => {
      // Второй animation frame — после того как капсула отрендерилась
      requestAnimationFrame(() => {
        console.log('🎯 Попытка фокуса после рендеринга');
        
        // Пробуем сфокусироваться
        try {
          userInput.focus();
          console.log('✅ focus() вызван');
          
          // Проверяем, сработал ли фокус
          setTimeout(() => {
            if (document.activeElement === userInput) {
              ...
              // Повторная попытка через setTimeout (для некоторых мобильных браузеров)
              setTimeout(() => {
    ...
    // 8. ✅ ИСПРАВЛЕНО: Убираем tg.showKeyboard() — его нет в API!
    //    Клавиатура покажется автоматически при фокусе на поле ввода
    //    Дополнительно пробуем скроллить к полю ввода
    try {
