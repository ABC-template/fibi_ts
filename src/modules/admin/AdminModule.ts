...
import { referralStore } from '@/modules/referral/ReferralStore';
import { sponsorsStore, type ISponsorTask } from '@/modules/sponsors/SponsorsStore';
...
type AdminTab = 'dashboard' | 'tasks' | 'coins' | 'referrals' | 'users';
...
export class AdminModule {
  ...
  async init(): Promise<void> {
    ...
    // Проверяем, что пользователь — creator
    ...
    console.log('✅ AdminModule v1.0.0 инициализирован');
  }
  ...
  private _renderTasksTab(): string {
    const tasks = this.sponsorsStore.getAllTasks();
    ...
    return `
      ...
      <div style="display: flex; flex-direction: column; gap: 10px;">
        ...
        ` : tasks.map(task => this._renderTaskItem(task)).join('')}
      </div>
    `;
  }

  private _renderTaskForm(task?: ISponsorTask): string {
    const t = task || {} as ISponsorTask;

    return `
      <div style="display: flex; flex-direction: column; gap: 8px;">
        <input id="admin-task-title" value="${t.title || ''}" placeholder="Название задания" style="
          padding: 10px;
          border-radius: 8px;
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
          <input id="admin-task-sponsor" value="${t.sponsor_name || ''}" placeholder="Спонсор" style="
            padding: 10px;
            border-radius: 8px;
            border: 1px solid var(--app-border-color);
            background: var(--app-bg-tertiary);
            ...
            outline: none;
          ">
          <input id="admin-task-reward" type="number" value="${t.reward || 10}" placeholder="Награда (монеты)" style="
            padding: 10px;
            border-radius: 8px;
            border: 1px solid var(--app-border-color);
            background: var(--app-bg-tertiary);
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px;">
          <select id="admin-task-type" style="
            ...
            outline: none;
          ">
            <option value="subscribe" ${t.type === 'subscribe' ? 'selected' : ''}>📢 Подписка</option>
            <option value="visit" ${t.type === 'visit' ? 'selected' : ''}>🌐 Переход</option>
            <option value="action" ${t.type === 'action' ? 'selected' : ''}>⚡ Действие</option>
            <option value="survey" ${t.type === 'survey' ? 'selected' : ''}>📝 Опрос</option>
          </select>
          <input id="admin-task-target" value="${t.target || ''}" placeholder="Цель (канал/сайт)" style="
            padding: 10px;
            border-radius: 8px;
            border: 1px solid var(--app-border-color);
            background: var(--app-bg-tertiary);
            ...
            outline: none;
          ">
          <input id="admin-task-action" value="${t.action_required || ''}" placeholder="Что сделать" style="
            padding: 10px;
            border-radius: 8px;
            border: 1px solid var(--app-border-color);
            background: var(--app-bg-tertiary);
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px;">
          <select id="admin-task-verification" style="
            ...
            outline: none;
          ">
            <option value="auto" ${t.verification_type === 'auto' ? 'selected' : ''}>🔍 Авто</option>
            <option value="pseudo" ${t.verification_type === 'pseudo' || !t.verification_type ? 'selected' : ''}>⏳ Псевдо (12ч)</option>
            <option value="manual" ${t.verification_type === 'manual' ? 'selected' : ''}>👤 Ручная</option>
          </select>
          <input id="admin-task-pseudo-hours" type="number" value="${t.pseudo_hours || 12}" placeholder="Часов проверки" style="
            padding: 10px;
            border-radius: 8px;
            border: 1px solid var(--app-border-color);
            background: var(--app-bg-tertiary);
            ...
            outline: none;
          ">
          <input id="admin-task-max-completions" type="number" value="${t.max_completions || ''}" placeholder="Лимит выполнений" style="
            padding: 10px;
            border-radius: 8px;
            border: 1px solid var(--app-border-color);
            background: var(--app-bg-tertiary);
        ...
        <div style="display: flex; gap: 8px;">
          <input id="admin-task-expires" type="datetime-local" value="${t.expires_at ? new Date(t.expires_at).toISOString().slice(0, 16) : ''}" style="
            flex: 1;
            padding: 10px;
            border-radius: 8px;
            border: 1px solid var(--app-border-color);
            ...
            outline: none;
          ">
          <button onclick="window.adminModule.saveTask('${t.id || ''}')" style="
            padding: 10px 24px;
            border-radius: 8px;
            border: none;
  ...
  private _renderTaskItem(task: ISponsorTask): string {
    const status = task.is_active ? '🟢 Активно' : '🔴 Неактивно';
    const statusColor = task.is_active ? '#27ae60' : '#e74c3c';
        ...
        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
          <div style="flex: 1; min-width: 0;">
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 4px 8px; margin-top: 6px;">
              ...
              <span style="font-size: 10px; color: ${statusColor}; background: ${task.is_active ? 'rgba(39, 174, 96, 0.1)' : 'rgba(231, 76, 60, 0.1)'}; padding: 2px 8px; border-radius: 10px;">
                ${status}
              </span>
            </div>
          </div>
  ...
  async editTask(taskId: string): Promise<void> {
    ...
    // Сохраняем ID задания в атрибуте кнопки
  ...
  async manageCoins(action: 'add' | 'spend'): Promise<void> {
    ...
    try {
      const response = await fetch(`/api/admin/coins/${action}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Telegram-Init-Data': (window as any).Telegram?.WebApp?.initData || '',
  ...
  private _renderUsersTab(): string {
    return `
      <div style="background: var(--app-bg-secondary); border-radius: 12px; padding: 16px; border: 1px solid var(--app-border-color-light);">
        ...
        <div style="margin-top: 8px; font-size: 11px; color: var(--app-text-tertiary);">
          ⚠️ Изменение роли влияет на доступ к функциям приложения
        </div>
      </div>
    `;
  }
  ...
  show(): void {
    ...
    // Загружаем статистику, если её нет
    if (!this.adminStore.getStats()) {
      this.loadStats();
    }
...
console.log('✅ AdminModule v1.0.0 загружен');
