...
export class QuestsModule {
  ...
  private _renderQuestItem(quest: IUserQuest): string {
    ...
    if (quest.type === 'sponsor' && quest.status === 'pending') {
      actionButton = `
        <button class="quests-action-btn primary" onclick="window.questsModule.submitProof('${quest.user_quest_id}')">
          📤 Отправить на проверку
        </button>
      `;
    } else if (quest.type === 'sponsor' && quest.status === 'submitted') {
      actionButton = `
        <div class="quests-status-badge" style="color: ${status.color};">
          ${status.label}
          ${quest.expires_at ? `⏱️ до ${new Date(quest.expires_at).toLocaleTimeString()}` : ''}
        </div>
      `;
    } else if (quest.type === 'sponsor' && quest.status === 'approved' && !quest.claimed) {
      actionButton = `
    ...
    return `
      <div class="quests-item ${isCompleted ? 'completed' : ''}">
  ...
  async submitProof(userQuestId: string): Promise<void> {
    ...
    if (success) {
      this.render();
      this.uiRenderer.showToast('📤 Заявка отправлена на проверку!', 'success', 2000);
    } else {
